export default class MapaAgentes {
  constructor() {
    ...
    this.keyMapa = new Map(); // cada alias apunta a una clave principal
  }
  ...
  set(keys, value) {
    ...
    /* A nuestro keyMapa, agregamos cada una de las llaves que pueden apuntar a este agente(pero apuntan a la llave de mapaGeneral)
    Y asignamos diversas keys a esta key */
    for (const key of keys) {
      this.keyMapa.set(key, mainKey);
    }
  }
  ...
  /**
   * Asigna un array de Keys a un value(Agente)
   * Se pasa cualquier key y primero busca en el keyMapa, para saber que esta llave a quien puede apuntar.
   * Y con esta nueva mainKey ahora si busca en el MapaGeneral. Y devuelve el Agente.
   * @param {String} key
   * @return {Object} Agente
   */
  get(key) {
    const mainKey = this.keyMapa.get(key);
